...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if (uCode >= 65 && uCode <= 90) {
            if (uCode <= 77) massN.push(String.fromCharCode(uCode + 13));
            else massN.push(String.fromCharCode(uCode - 13));
            continue;
        }

        if (uCode >= 97 && uCode <= 122) {
            if (uCode <= 109) massN.push(String.fromCharCode(uCode + 13));
            else massN.push(String.fromCharCode(uCode - 13));
            continue;
        }
...
function getCardId(value) {
    ...
    if (value.length === 3) {
        return suits[value.charCodeAt(2)] + 9;
    }
    return suits[value.charCodeAt(1)] + Number(chars[value[0]] || value[0]) - 1;
}
